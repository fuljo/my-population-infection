...
\begin{algorithm}[p]
    ...
    \For{$t \leftarrow 0$ \KwTo $t_{target}$ \Step $t_{step}$}{
        \ForEach{subsceptible individual $i$, infected individual $j$}{
            \UpdateExposure{$i,j$}\;
        }
        \ForEach{individual $i$}{
            \UpdateStatus{i}\;
        ...
        \If{end of day}{
            $\langle subsceptible_c, infected_c, immune_c \rangle \leftarrow$ \CountByStatus{$\left\{ status_{i} \right\}$}\;
            $summary \leftarrow$ \MPIGather{$\left\{\langle subsceptible_c, infected_c, immune_c \rangle\right\}$}\;
            \If{root}{
                \WriteToCSV{$summary$}\;
            }
        }
